...
import { EndCallMessageCard } from "./EndCallMessageCard";
...
const MessageCard = ({
  ...
  // Check if message is end call message
  const isEndCallMessage = chat.text?.startsWith("//Cuoc goi ket thuc");
  ...
  return (
    <>
      ...
      {/* Render EndCallMessageCard for call end messages */}
      {isEndCallMessage ? (
        <EndCallMessageCard
          key={chat.id}
          message={chat.text}
          timestamp={format(new Date(chat.createAt), "HH:mm")}
          isSender={isSender}
          createAvatar={chat.createAvatar}
          createName={chat.createName}
          showName={!isSender && item?.groupName !== chat.createName}
        />
      ) : (
        <div
          className={`flex flex-col ${isSender ? "items-end" : "items-start"} mb-4`}
